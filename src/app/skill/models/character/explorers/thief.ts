import { SkillData } from "../../skill-data";
const nightLordSkillData:SkillData[]=[{
    name: '쇼다운 챌린지',
    location: 'assets/showdown.png',
    job: '나이트로드',
    lt: {
      x: -480,
      y: -220,
    },
    rb: {
      x: 5,
      y: 40,
    },
    origin: {
      x: 616,
      y: 300,
    },
  },
  {
    name: '다크 플레어',
    location: 'assets/darkflare.png',
    job: '나이트로드',
    lt: {
      x: -399,
      y: -280,
    },
    rb: {
      x: 401,
      y: 140,
    },
    origin: {
      x: 442,
      y: 326,
    },
  },
  {
    name: '써든레이드',
    location: 'assets/suddenraid.png',
    job: '나이트로드',
    lt: {
      x: -400,
      y: -350,
    },
    rb: {
      x: 400,
      y: 250,
    },
    origin: {
      x: 465,
      y: 426,
    },
  },
  {
    name: '포 시즌',
    location: 'assets/deathstar.png',
    job: '나이트로드',
    lt: {
      x: -300,
      y: -330,
    },
    rb: {
      x: 300,
      y: 50,
    },
    origin: {
      x: 366,
      y: 360,
    },
  },]
const ShadowerSkillData:SkillData[]=[
  {
    name: '암살(1타)',
    location: 'assets/amsal1.png',
    job: '섀도어',
    lt: {
      x: -275,
      y: -100,
    },
    rb: {
      x: -10,
      y: 30,
    },
    origin: {
      x: 331,
      y: 184,
    },
  },{
    name: '암살(2타)',
    location: 'assets/amsal2.png',
    job: '섀도어',
    lt: {
      x: -335,
      y: -200,
    },
    rb: {
      x: -10,
      y: 30,
    },
    origin: {
      x: 453,
      y: 402,
    },
  },
  {
    name: '암살(2타, 살의)',
    location: 'assets/amsal2_kill.png',
    job: '섀도어',
    lt: {
      x: -375,
      y: -250,
    },
    rb: {
      x: -10,
      y: 30,
    },
    origin: {
      x: 494,
      y: 428,
    },
  },
  {
    name: '크루얼 스탭',
    location: 'assets/cruelstab.png',
    job: '섀도어',
    lt: {
      x: -440,
      y: -175,
    },
    rb: {
      x: 40,
      y: 55,
    },
    origin: {
      x: 538,
      y: 184,
    },
  },
  {
    name: '연막탄',
    location: 'assets/smoke.png',
    job: '섀도어',
    lt: {
      x: -365,
      y: -320,
    },
    rb: {
      x: 275,
      y: 80,
    },
    origin: {
      x: 386,
      y: 357,
    },
  },
  {
    name: '베일 오브 섀도우',
    location: 'assets/bailofshadow.png',
    job: '섀도어',
    lt: {
      x: -420,
      y: -620,
    },
    rb: {
      x: 420,
      y: 120,
    },
    origin: {
      x: 531,
      y: 756,
    },
  },
  {
    name: '소닉 블로우',
    location: 'assets/sonicblow.png',
    job: '섀도어',
    lt: {
      x: -500,
      y: -250,
    },
    rb: {
      x: 120,
      y: 50,
    },
    origin: {
      x: 473,
      y: 265,
    },
  },
  {
    name: '암살 VI(1타)',
    location: 'assets/explorers/thief/6thAssassiantion1.png',
    job: '섀도어',
    lt: {
      x: -315,
      y: -115,
    },
    rb: {
      x: 15,
      y: 35,
    },
    origin: {
      x: 528,
      y: 250,
    },
  },{
    name: '암살 VI(2타)',
    location: 'assets/explorers/thief/6thAssassiantion2.png',
    job: '섀도어',
    lt: {
      x: -350,
      y: -210,
    },
    rb: {
      x: 0,
      y: 35,
    },
    lt2: {
        x: -390,
        y: -260,
      },
    rb2: {
        x: 0,
        y: 35,
      },
    origin: {
      x: 448,
      y: 408,
    },
  },
  {
    name: '분쇄 VI(1타)',
    location: 'assets/explorers/thief/6thHeartbreaker.png',
    job: '섀도어',
    lt: {
      x: -390,
      y: -165,
    },
    rb: {
      x: 15,
      y: 35,
    },
    origin: {
      x: 498,
      y: 150,
    },
  },  {
    name: '분쇄 VI(2타)',
    location: 'assets/explorers/thief/6thHeartbreaker2.png',
    job: '섀도어',
    lt: {
      x: -395,
      y: -285,
    },
    rb: {
      x: 10,
      y: 35,
    },
    origin: {
      x: 691,
      y: 472,
    },
  },]
const DualBladeSkillData:SkillData[]=[{
    name: '블레이드 퓨리',
    location: 'assets/bladefury.png',
    job: '듀얼블레이드',
    lt: {
      x: -260,
      y: -240,
    },
    rb: {
      x: 260,
      y: 10,
    },
    origin: {
      x: 316,
      y: 212,
    },
  },
  {
    name: '팬텀 블로우',
    location: 'assets/phantomblow.png',
    job: '듀얼블레이드',
    lt: {
      x: -350,
      y: -200,
    },
    rb: {
      x: 10,
      y: 70,
    },
    origin: {
      x: 421,
      y: 175,
    },
  },
  {
    name: '아수라',
    location: 'assets/asura.png',
    job: '듀얼블레이드',
    lt: {
      x: -220,
      y: -250,
    },
    rb: {
      x: 200,
      y: 30,
    },
    origin: {
      x: 299,
      y: 285,
    },
  },
  {
    name: '블레이드 스톰(개시)',
    location: 'assets/bladestorminit.png',
    job: '듀얼블레이드',
    lt: {
      x: -665,
      y: -250,
    },
    rb: {
      x: 0,
      y: 45,
    },
    origin: {
      x: 802,
      y: 482,
    },
  },
  {
    name: '블레이드 스톰(키다운)',
    location: 'assets/bladestormrepeat.png',
    job: '듀얼블레이드',
    lt: {
      x: -515,
      y: -255,
    },
    rb: {
      x: 20,
      y: 50,
    },
    origin: {
      x: 619,
      y: 297,
    },
  },
  {
    name: '카르마 퓨리',
    location: 'assets/karmafury.png',
    job: '듀얼블레이드',
    lt: {
      x: -460,
      y: -530,
    },
    rb: {
      x: 460,
      y: 60,
    },
    origin: {
      x: 479,
      y: 541,
    },
  },
  {
    name: '블레이드 토네이도',
    location: 'assets/bladetornado.png',
    job: '듀얼블레이드',
    lt: {
      x: -270,
      y: -450,
    },
    rb: {
      x: 270,
      y: 5,
    },
    origin: {
      x: 260,
      y: 431,
    },
  },
  {
    name: '블레이드 토네이도(태풍)',
    location: 'assets/bladetornado_shoot.png',
    job: '듀얼블레이드',
    lt: {
      x: 0 - 200 / 2 - 550,
      y: -185 - 370 / 2,
    },
    rb: {
      x: 0 + 200 / 2,
      y: -185 + 370 / 2,
    },
    origin: {
      x: 150 + 0 + 550,
      y: 258 + 185,
    },
  },
  {
    name: '헌티드 엣지-나찰',
    location: 'assets/hauntededge_nachal.png',
    job: '듀얼블레이드',
    lt: {
      x: -430,
      y: -205,
    },
    rb: {
      x: 20,
      y: 80,
    },
    origin: {
      x: 475,
      y: 249,
    },
  },
  {
    name: '팬텀 블로우 VI',
    location: '/assets/explorers/thief/6thPhantomblow.png',
    job: '듀얼블레이드',
    lt: {
      x: -405,
      y: -205,
    },
    rb: {
      x: 25,
      y: 80,
    },
    origin: {
      x: 535,
      y: 298,
    },
  },]
const thiefCommonSkillData:SkillData[]=[]
export const thiefSkillData:SkillData[]=[...nightLordSkillData,...ShadowerSkillData,...DualBladeSkillData,...thiefCommonSkillData]